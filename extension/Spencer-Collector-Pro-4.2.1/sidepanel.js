...
async function reset() {
  ...
  ["scanBar", "metaBar", "downloadBar", "driveBar"].forEach(id => $(id).style.width = "0%");
  ...
  $("driveProgress").textContent = "Conexión sin verificar";
  $("driveEta").textContent = "";
  ...
  $("collectionPreview").innerHTML = 'ID: se asignará automáticamente al subir.<br>Formato visible: <strong>WMC-1 - Pokémon</strong>';
  $("scan").disabled = false;
...
/* =========================================================
   PASO 5 - GOOGLE DRIVE / COLECCIONES
========================================================= */

function driveCandidate(item) {
  return {
    url: item.url,
    alternateUrls: Array.isArray(item.alternateUrls) ? item.alternateUrls : [],
    previewUrl: item.previewUrl || "",
...
function updateCollectionPreview() {
  const normalized = normalizeCollectionName($("collectionName").value);
  if (normalized && $("collectionName").value !== normalized) {
    // Solo mostramos la versión normalizada; se aplica definitivamente al salir/subir.
  }
  const rawId = currentCollectionId || "WMC-1";
  const visibleId = compactCollectionId(rawId);
  $("collectionPreview").innerHTML = normalized
    ? `Nombre normalizado: <strong>${normalized}</strong><br>ID visible: <strong>${visibleId}</strong><br>Nombre visible sugerido: <strong>${formatCollectionFolderName(rawId, normalized)}</strong><br>Se crearán subcarpetas dentro de JPG/PNG/GIF/WEBP/VIDEO según corresponda.`
    : 'ID: se asignará automáticamente al subir.<br>Escribe un nombre, por ejemplo <strong>Pokémon</strong>.';
  updateDriveUploadState();
}

function updateDriveUploadState() {
  const selected = getSelectedMedia();
  const name = normalizeCollectionName($("collectionName")?.value || "");
  if ($("uploadDrive")) $("uploadDrive").disabled =
    uploadingDrive || !driveVerified || !analysisReady || !selected.length || !name;
}

$("collectionName").addEventListener("input", () => {
  const normalized = normalizeCollectionName($("collectionName").value);
  if (currentCollectionId && normalized !== currentCollectionName) currentCollectionId = null;
...
$("verifyDrive").addEventListener("click", async () => {
  if (uploadingDrive) return;
  $("verifyDrive").disabled = true;
  $("driveBar").style.width = "20%";
  $("driveProgress").textContent = "Verificando conexión con Google Drive...";
  $("driveEta").textContent = "No se subirá ningún archivo durante esta verificación.";

  try {
    await requireCompatibleServer();
    const data = await ensureDriveAuthorization();
    driveVerified = true;
    $("driveBar").style.width = "100%";
    $("driveProgress").innerHTML = '<span class="success">✓ Google Drive conectado correctamente</span>';
    $("driveEta").textContent = "Ahora escribe el nombre de la colección y sube la selección.";
    $("collectionName").disabled = false;
    log("Conexión con Google Drive verificada.");
  } catch (error) {
    driveVerified = false;
    $("driveBar").style.width = "0%";
    $("driveProgress").innerHTML = '<span class="warning">No se pudo verificar Google Drive.</span>';
    $("driveEta").textContent = explainServerError(error);
    updateServerStatus();
    $("collectionName").disabled = true;
    log(`ERROR conexión Drive: ${error.message}`);
  } finally {
    $("verifyDrive").disabled = false;
    updateDriveUploadState();
  }
});

$("uploadDrive").addEventListener("click", async () => {
  const selected = getSelectedMedia();
  if (uploadingDrive || !selected.length || !analysisReady || !driveVerified) return;

  const normalizedName = normalizeCollectionName($("collectionName").value);
  if (!normalizedName) {
    $("driveProgress").innerHTML = '<span class="warning">Escribe el nombre de la colección antes de subir.</span>';
    return;
  }

  $("collectionName").value = normalizedName;
  if (currentCollectionId && currentCollectionName !== normalizedName) currentCollectionId = null;

  uploadingDrive = true;
  $("uploadDrive").disabled = true;
  $("verifyDrive").disabled = true;
  $("collectionName").disabled = true;
  $("driveBar").style.width = "2%";
  $("driveProgress").textContent = `Preparando subida a Google Drive... 0/${selected.length} · 2%`;
  $("driveEta").textContent = "Creando trabajo de subida en la nube...";

  try {
    await requireCompatibleServer();
    const response = await authFetch("/api/ingest/start", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        candidates: selected.map(driveCandidate),
        collectionName: normalizedName,
        collectionId: currentCollectionId
      })
    });
    const startData = await response.json();
    if (!response.ok) throw new Error(startData.error || `HTTP ${response.status}`);

    currentCollectionId = startData.collectionId;
    currentCollectionName = startData.collectionName;
    $("collectionName").value = startData.collectionName;
    updateCollectionPreview();

    const data = await waitForIngestJob(startData.jobId, selected.length);
    currentCollectionId = data.collectionId;
    currentCollectionName = data.collectionName;
    $("collectionName").value = data.collectionName;
    $("driveBar").style.width = "100%";
    const visibleFolder = data.folderLabel || formatCollectionFolderName(data.collectionId, data.collectionName);
    $("driveProgress").innerHTML =
      `<span class="success">✓ ${visibleFolder}: 100% · ${data.uploaded} nuevos · ${data.duplicates} duplicados · ${data.failed} fallos</span>`;
    $("driveEta").textContent = `Procesados: ${data.processed} / ${data.total} · Tiempo total: ${formatDuration(data.elapsedMs || 0)}.`;
    updateCollectionPreview();
    log(`Drive: ${visibleFolder}. Nuevos: ${data.uploaded}; duplicados: ${data.duplicates}; fallos: ${data.failed}.`);
  } catch (error) {
    $("driveBar").style.width = "0%";
    $("driveProgress").innerHTML = '<span class="warning">No se pudo completar la subida.</span>';
    const elapsedLabel = driveUploadStartedAt ? formatDuration(Date.now() - driveUploadStartedAt) : "00:00";
    $("driveEta").textContent = error.message === "AUTH_REQUIRED"
      ? `La sesión de Google expiró. Pulsa verificar conexión para autorizar de nuevo. Tiempo transcurrido: ${elapsedLabel}.`
      : `Los recursos que no puedan descargarse se reportarán como fallos sin cancelar los demás. Tiempo transcurrido: ${elapsedLabel}.`;
    updateServerStatus();
    log(`ERROR Drive: ${error.message}`);
  } finally {
    driveUploadStartedAt = 0;
    uploadingDrive = false;
    $("verifyDrive").disabled = false;
    $("collectionName").disabled = !driveVerified;
    updateDriveUploadState();
  }
});
